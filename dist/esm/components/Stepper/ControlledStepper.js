...
import { arrayOf, shape, string, number, node, oneOfType, func, bool } from 'prop-types';
...
export class ControlledStepper extends PureComponent {
  constructor(...args) {
    ...
    _defineProperty(this, "renderNavigationStep", ({
      id,
      label,
      disabled
    }) => {
      ...
      const classes = classnames(`${OLT_NAMESPACE}Stepper-header`, id === currentStepIdx && 'is-active');
    ...
    _defineProperty(this, "renderContent", ({
      ...
      const classes = classnames(`${OLT_NAMESPACE}Stepper-content`, id === currentStepIdx && 'is-active');
      const contentBody = typeof content === 'function' ? content({
  ...
  render() {
    ...
    return React.createElement("section", {
      className: classes
    }, steps.map((x, i) => React.createElement(Fragment, {
      key: `${x.id}`
    }, this.renderNavigationStep(x), this.renderContent(x))));
  }
