  ...
  render() {
    const _this$props = this.props,
      ...
      color
          ...
          props = _objectWithoutProperties(_this$props, ["regular", "tag", "children", "size", "className", "color"]);

    const Element = tag || `h${size}`;
    return React.createElement(Element, _extends({}, props, {
      className: classnames(`${OLT_NAMESPACE}Headline`, `${OLT_NAMESPACE}Headline--${size}`, regular && `${OLT_NAMESPACE}Headline--regular`, color && `${OLT_NAMESPACE}u-text${pascalize(color)}`, className)
    }), children);
  }
