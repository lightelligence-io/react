...
import { BasicDataCardsCardItem } from '../BasicDataCards';

export const DataCardsItem = ({
  withoutTitle,
  render,
  renderProps,
  data,
  ...
  return (
    <BasicDataCardsCardItem title={finalTitle} {...props}>
      {content}
    </BasicDataCardsCardItem>
  );
};

DataCardsItem.propTypes = {
  /**
   * The `render` function receives on object with two fields and should return
   * the content of the currently rendered `BasicDataCardsCardItem`.
   ...
   * an object which is forwarded as `props` to the underlying `BasicDataCardsCardItem` component.
   */
  renderProps: func,
  /**
   * This **internal** property is provided by the parent `DataCards` component directly.
