```js
import { Filter, Checkbox, Button, Card } from '@lightelligence/react';
initialState = {
  active: false,
  open: false,
  filterItems: {},
...
<Card color="light" style={{ flex: 1, flexDirection: 'row' }}>
  <Filter
    ...
    content={
      <>
        <Checkbox
          checked={state.filterItems.brightness}
          onChange={onChange('brightness')}
        >
          Brightness
        </Checkbox>
        <Checkbox
          checked={state.filterItems.Humidity}
          onChange={onChange('Humidity')}
        >
          Humidity
        </Checkbox>
        <Checkbox
          checked={state.filterItems.occupancy}
          onChange={onChange('occupancy')}
        >
          Occupancy
        </Checkbox>
        <Checkbox
          checked={state.filterItems.power}
    ...
    actions={[
      <Button key="apply" buttonType="action" onClick={onApply}>
        Apply
      </Button>,
    ]}
...
```js
import { Filter, Checkbox, Button } from '@lightelligence/react';
initialState = {
  active: false,
  open: false,
  filterItems: {},
...
<Filter
  ...
  actions={[
    <Button key="clear" emphasis="tertiary" onClick={handleClear}>
      Clear
    </Button>,
    <Button key="reset" emphasis="tertiary" onClick={handleReset}>
      Reset
    </Button>,
    <Button key="apply" buttonType="action" onClick={handleApply}>
      Apply
    </Button>,
  ]}
/>;
