...
const renderComponent = ({ testId = COMPONENT_TEST_ID, ...props }) => {
  return render(<Toggle data-testid={testId} {...props} />);
};
...
describe('Toggle', () => {
  ...
  test('forwards className', () => {
    ...
    expect(toggle.classList.contains('myClass')).toBeTruthy();
  });
  ...
  test('correctly sets the css-modifier for inline', () => {
    ...
    expect(button.classList.contains(oltStyles.ToggleInline)).toBeTruthy();
  });

  test('correctly sets the css base class', () => {
    const { getByTestId } = renderComponent({});
    const button = getByTestId(COMPONENT_TEST_ID);
    expect(button.classList.contains(oltStyles.Toggle)).toBeTruthy();
  });
});
